...
BINARY_SEARCH_PATTERN = 'B8 ?? ?? ?? ?? F7' # mov eax, xxxxx
...
def main():
    ...
    while pos < end_segm:
        ...
        print("Pattern Position [0x%08X]" % pos)
        if pos > end_segm:
            break
        ...
        if (mnem == 'mov' and opnd0 == 'eax'):
            func_start = pos
            pos = idc.next_head(pos)
            mnem = idc.print_insn_mnem(pos)
            opnd0 = idc.print_operand(pos, 0)
            if (mnem == 'neg' and opnd0 == 'eax'):
                pos = idc.next_head(pos)
                mnem = idc.print_insn_mnem(pos)
                opnd0 = idc.print_operand(pos, 0)
                if (mnem == 'jmp' and opnd0 == 'eax'): # Found Pattern!
                    name_flag = False
                    ...
                    idc.auto_wait()
                    print("[0x%08X] API : %s Jumper Address: 0x%08X" % (api_addr, api_name2, func_start))
                else:
