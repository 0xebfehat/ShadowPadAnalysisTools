...
def clean_jmp_control_flow(searchPattern, startAddress, endAddress):
    ...
    while emu_start < endAddress:
        ...
        if (idc.is_code(idc.get_full_flags(emu_start)) == True):
            ...
            eip = emu_x86_.get_reg_value(UC_X86_REG_EIP)
            mnem = idc.print_insn_mnem(eip)
            byte_code = idc.get_wide_byte(eip)
            if byte_code == 0x90:
                msg = "[Emulation Start] Found jmp xxxxx 0x%08X %s" % (emu_start, idc.generate_disasm_line(emu_start,0))
                print(msg)
                step_cnt = 0
                while step_cnt < MAX_EMU_CNT:
                    emu_x86_.eUntilAddress(eip, 0, Count=1)
                    disasm = idc.generate_disasm_line(eip,0)
                    mnem = idc.print_insn_mnem(eip)
                    byte_code = idc.get_wide_byte(eip) 
                    if byte_code == 0xE9:
                        for i in range(5):
                            idc.patch_byte(eip + i, 0x90)
                    idc.auto_wait()
                    if (byte_code != 0x90) and (byte_code != 0xE9):
                        target_address = (eip - (emu_start + 5)) & 0xFFFFFFFF
                        msg = "new jmp target (Near Relative) 0x%08X (0x%08X)" % (target_address, eip)
                        print(msg)
