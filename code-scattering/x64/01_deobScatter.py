...
redirect_func_address =  <jump function address> # e.g. 0x180015855
...
def deobs_code_obfuscate(searchPattern, startAddress, endAddress):
    ...
    while pos < endAddress:
        ...
        func_addr = pos + 0x5 +  (0xFFFFFFFF << 32 | idaapi.get_dword(pos + 1))
        func_addr = 0x100000000 | func_addr
        func_addr = func_addr & 0x0FFFFFFFFFFFFFFF
        msg = "func_addr 0x{:08X}".format(func_addr)
        print(msg)

        if func_addr == redirect_func_address:
            addr = idaapi.get_qword(pos + 5)
